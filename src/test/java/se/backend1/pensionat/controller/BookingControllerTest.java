...
public class BookingControllerTest {
    ...
    public void testGetAvailableRooms() throws Exception {
        when(roomService.findAvailableRooms(any(LocalDate.class), any(LocalDate.class), any(Integer.class)))
                .thenReturn(List.of(new RoomDto()));

        mockMvc.perform(get("/bookings/search-results")
                        .param("checkIn", "2025-05-01")
                        .param("checkOut", "2025-05-05")
                        .param("guests", "2"))
                .andExpect(status().isOk())
                .andExpect(view().name("bookings/search-results"))
                .andExpect(model().attributeExists("availableRooms"));
    }
    @Test
    public void testCreateBooking_Success() throws Exception {
        // Inga valideringsfel → bookingService.save() ska anropas (vi mockar inget speciellt för save)
    ...
    @Test
    public void testUpdateBooking_ValidationError() throws Exception {
        when(customerService.getAllCustomers()).thenReturn(List.of());
        when(roomService.getAllRooms()).thenReturn(List.of());

        mockMvc.perform(post("/bookings/edit/1")
                        .contentType(MediaType.APPLICATION_FORM_URLENCODED)
                        // Skicka utan checkIn för att trigga valideringsfel
                        .param("checkOut", "2025-05-05")
                        .param("numberOfGuests", "2")
                )
                .andExpect(status().isOk())
