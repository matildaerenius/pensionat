...
public class CustomerControllerTest {
    ...
    public void testShowEditForm() throws Exception {
        ...
        // Returnera exakt samma objekt båda gångerna
        when(customerService.getCustomerById(1L)).thenReturn(dto);

        mockMvc.perform(get("/customers/edit/1"))
                .andExpect(status().isOk())
                .andExpect(view().name("customers/form"))
                .andExpect(model().attribute("customerDto", dto))
                .andExpect(model().attribute("edit", true))
                .andExpect(model().attribute("formAction", "/customers/edit/1"));

        verify(customerService, times(2)).getCustomerById(1L); // eftersom det anropas två gånger
    }
